...
const MyProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const myProfile = res.data.find(emp => emp.email === user.email);
        setProfile(myProfile || res.data[0]);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  if (!profile) return <div><Navbar /><div className={styles.container}>Profile not found</div></div>;
